...
while True:
    ...
    if ticker >= 5:
        ...
        hardware.led_in_bedrijf.value = logica.status_in_bedrijf
        ticker = 0
        
    if logica.status_hoog_urgente_storing == 1:
        hardware.led_hoog_urgent.value = lh_state
        
    if logica.status_laag_urgente_storing == 1:
        hardware.led_laag_urgent.value = lh_state
    
    if logica.status_hoog_urgente_storing == 0:
        hardware.led_hoog_urgent.value = 0
        
    if logica.status_laag_urgente_storing == 0:
    ...
    if logica.status_in_bedrijf == 1 and hardware.servo_status == 0:
        ticker_servo += settings.tijd_vertraging_servo 
    ...
    lcd_setpoint = "setpoint: " + str(setpoint_temperatuur.temp) + " *C"  
    lcd_string(lcd_setpoint,LCD_LINE_1)
    lcd_temp = "temperatuur: " + str(huidige_temperatuur.temp) + " *C" 
    lcd_string(lcd_temp,LCD_LINE_2)
    
    if logica.status_in_bedrijf == 1 and logica.status_hoog_urgente_storing == 0 and hardware.servo_status == 1: 
    ...
    if logica.status_hoog_urgente_storing == 1:
        heater_controller = HeaterController(10, 1, 0, 0.25)
        publish_message(topic_prefix + "status_regelaar", "neutraal")
        publish_message(topic_prefix + "verwarm_factor", 0)
        publish_message(topic_prefix + "koel_factor", 0)
